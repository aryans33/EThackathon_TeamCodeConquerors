...
const CATEGORY_BADGES: Record<string, { bg: string; text: string }> = {
  ...
  'Bulk Deal': { bg: '#000000', text: '#d4af37' },
    ...
    <main className="max-w-7xl mx-auto p-4 md:p-6 space-y-6 dark:bg-black bg-gray-50 transition-colors min-h-screen">
            ...
            {filteredSearch.length === 0 ? (
             ...
             filteredSearch.map(s => (
              <button
                ...
                className="w-full text-left flex items-center justify-between p-3 dark:hover:bg-[#22314a] hover:bg-gray-100 rounded-lg transition-colors cursor-pointer"
              >
                <div>
                  <span className="font-bold dark:text-[#d4af37] text-[#d4af37]">{s.symbol}</span>
                  <span className="ml-3 dark:text-[#9ca3af] text-gray-700">{s.name}</span>
                </div>
                {s.sector && <span className="text-xs dark:bg-[#22314a] bg-gray-200 dark:text-[#64748b] text-gray-700 px-2 py-1 rounded-full">{s.sector}</span>}
              </button>
             ))
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-6">
        ...
        <ErrorBoundary section="Signals">
          <div className="space-y-4">
            <div id="live-signals-section" className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold dark:text-[#f0fdf4] text-[#1f2937] flex items-center gap-2">
                  Live Signals <span className="w-2 h-2 rounded-full bg-[#d4af37] animate-pulse mt-1" />
                  <span className="text-xs font-normal dark:bg-[#22314a] bg-gray-200 dark:text-[#64748b] text-gray-700 px-2 py-0.5 rounded-full ml-2">
                    {filteredSignals.length}
                  </span>
                </h2>
                <div className="mt-1 text-xs dark:text-[#64748b] text-gray-600 flex items-center gap-2">
            ...
            {backendError && (
              <div className="dark:bg-amber-900/20 bg-amber-100 dark:text-amber-500 text-amber-700 dark:border-amber-800/50 border-amber-300 border rounded-xl p-4 text-center">
            ...
            <div className="space-y-4">
              {isLoadingSignals ? (
                ...
                <div className="dark:text-[#64748b] text-gray-600 text-center py-8">No signals matching filter.</div>
              ) : (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-3">
                        <span className="font-bold dark:text-white text-[#1f2937] text-lg">{signal.stock.symbol}</span>
                        <span className="text-sm dark:text-[#64748b] text-gray-600">{signal.stock.name}</span>
                      </div>
                      <div>
                        <span
                    ...
                    <div className="mb-4">
                      <span className="text-xs dark:bg-[#22314a] bg-gray-200 dark:text-[#64748b] text-gray-700 px-2 py-0.5 rounded-full capitalize">
                        {signal.signal_type.replace(/_/g, ' ')}
                      </span>
                      <p className="text-sm dark:text-[#e2e8f0] text-[#1f2937] mt-2 font-medium">{signal.one_line_summary}</p>
                      {signal.reason && <p className="text-xs dark:text-[#64748b] text-gray-600 mt-1">{signal.reason}</p>}
                    </div>
                    
                    <div className="flex items-center justify-between">
                      <div className="w-1/2 max-w-xs">
                        <div className="flex justify-between mb-1">
                          <span className="text-xs dark:text-[#64748b] text-gray-600">Confidence</span>
                          <span className="text-xs font-bold dark:text-[#64748b] text-gray-600">{signal.confidence}%</span>
                        </div>
                        <div id={idx === 0 ? 'confidence-bar' : undefined} className="w-full dark:bg-[#22314a] bg-gray-300 rounded-full h-1.5">
                          <div 
                            className={`h-1.5 rounded-full ${
                              signal.confidence > 70 ? 'bg-[#d4af37]' : signal.confidence >= 40 ? 'bg-amber-500' : 'bg-red-500'
                            }`} 
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-xs dark:text-[#64748b] text-gray-600">{timeAgo(signal.created_at)}</span>
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation()
        ...
        <div className="space-y-8">
          ...
          <ErrorBoundary section="Watchlist">
            <div>
              <div className="flex items-center justify-between mb-4 gap-3">
                <h3 className="text-lg font-bold dark:text-[#f0fdf4] text-[#1f2937]">Watchlist</h3>
                <span className="text-xs dark:text-[#64748b] text-gray-600">{watchlistRows.length} stocks</span>
              </div>
              <div className="dark:bg-[#101827] bg-white dark:border-[#22314a] border-gray-300 border rounded-xl p-3 mb-3">
                <div className="grid grid-cols-1 gap-2">
                  <select
                    value={watchlistPick}
                    onChange={(e) => setWatchlistPick(e.target.value)}
                    className="w-full min-w-0 dark:bg-[#0f172a] bg-white dark:border-[#22314a] border-gray-300 border rounded-lg px-3 py-2 text-sm dark:text-[#e2e8f0] text-[#1f2937]"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2 rounded-lg text-sm font-medium bg-[#d4af37] hover:bg-[#c79e16] disabled:opacity-50 disabled:cursor-not-allowed text-white"
                  >
                    Add
                  </button>
                </div>
              </div>
              <div className="dark:bg-[#101827] bg-white dark:border-[#22314a] border-gray-300 border rounded-xl p-3 max-h-[400px] overflow-y-auto">
                {pricesLoading ? (
                  <>
                    {Array.from({ length: 10 }).map((_, i) => (
                      <div key={i} className="h-10 rounded-lg bg-slate-300 dark:bg-slate-700 animate-pulse mb-2" />
                    ))}
                  </>
                ) : watchlistRows.length === 0 ? (
                  <p className="text-content-secondary text-sm">No stocks in watchlist. Add one above.</p>
                ) : (
                  <>
                    {watchlistRows.map((stock: any) => (
                      <div
                        ...
                        className="flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-[#1c2128] cursor-pointer transition-colors"
                      >
                        <div className="flex flex-col min-w-0">
                          <span className="text-content-primary font-semibold text-sm">{stock.symbol}</span>
                          <span className="text-content-secondary text-xs truncate max-w-[140px]">{stock.name}</span>
                        </div>
                        <div className="flex flex-col items-end shrink-0 ml-2">
                          {stock.latest_close ? (
                            <>
                              <span className="text-content-primary font-mono text-sm">
                        ...
                        <button
                          ...
                          className="ml-3 text-xs px-2 py-1 rounded-md dark:bg-[#1f2937] bg-gray-200 dark:text-[#94a3b8] text-gray-700 hover:bg-red-900/30 hover:text-red-300 transition-colors"
          ...
          {/* Today's Patterns */}
          <div>
            <h3 className="text-lg font-bold dark:text-[#f0fdf4] text-[#1f2937] mb-4">Today's Patterns</h3>
            <div className="dark:bg-[#101827] bg-white dark:border-[#22314a] border-gray-300 border rounded-xl p-5 space-y-3">
              {patternsLoading ? (
                <div className="space-y-2">
                  {[1, 2, 3].map(i => (
                    <div key={i} className="h-8 rounded-lg bg-slate-300 dark:bg-slate-700 animate-pulse" />
                  ))}
                </div>
              ) : patterns.filter(p => p.patterns?.some((pat: any) => pat.detected_today)).length === 0 ? (
                <>
                  <p className="text-content-secondary text-xs mb-2">No breakouts today</p>
